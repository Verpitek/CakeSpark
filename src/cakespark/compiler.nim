...
proc compile*(program: Program): CompiledProgram =
  ...
  for blockDef in program.blockDefs:
    if blockDef.kind != ankBlockDef:
      continue
    compileBlockDef(blockDef, blockC)
    if blockC.hasError:
      result.hasError = true
      result.errorMsg = blockC.errorMsg
      return
  ...
  for name, idx in blockC.blocks:
    var endIdx = blockC.instructions.len
    for i in idx + 1 ..< blockC.instructions.len:
      if blockC.instructions[i].kind == inkReturn and i > idx:
        endIdx = i + 1
        break
    result.blocks.add((name, blockC.instructions[idx ..< endIdx]))
